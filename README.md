...
- Issue 6：管理员导出全部有效记录为 Excel，导出结果与记录列表排序一致，普通员工无导出权限。
...
数据库结构通过 Prisma Migrate 创建，初始数据通过 Prisma seed 写入。当前 SQLite 环境使用 Prisma 官方 JS engine 与 `@prisma/adapter-better-sqlite3`，保留 Prisma schema、Client、migrate、seed 的标准使用方式。
...
仓库已包含 [render.yaml](/Users/duibagroup/Desktop/Vibe%20Coding/render.yaml:1) 可直接用于 Render Blueprint。由于当前数据库仍为 SQLite，线上部署需要挂载持久化磁盘；因此建议使用 Render `starter` 及以上方案，而不是无磁盘的免费实例。
